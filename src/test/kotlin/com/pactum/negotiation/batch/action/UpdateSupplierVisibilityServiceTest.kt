...
@UnitTest
class UpdateSupplierVisibilityServiceTest {
  ...
  @Test
  fun `can update supplier visibility of a list of negotiations`() {
    val negId1 = 1L
    val negId2 = 2L
    ...
    whenever(negotiationService.updateNegotiationSupplierVisibility(neg1, isVisibleSupplier, comment)).thenReturn(neg1)
    whenever(negotiationService.updateNegotiationSupplierVisibility(neg2, isVisibleSupplier, comment)).thenReturn(neg2)

    SessionHelper.setLoggedInUser(token, email, listOf(Role.Admin))

    val resp = updateSupplierVisibilityService.updateSupplierVisibility(batchReq)
